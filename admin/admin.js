...
async function loadData() {
  ...
  if (cachedListings.length) {
    ...
    // Fetch canonical published data — bypasses CDN cache for absolute latest state
    try {
    ...
    // Last resort: inline embed in this admin HTML
    if (!_listings.length) {
      const el = document.getElementById('listings-data')
      if (el) {
    ...
    if (_listings.length) {
      toast('⚠ Sesión nueva — datos del servidor. Cambios sin publicar de otro navegador no están aquí.', 'warning', 7000)
    }
  }
...
async function publishToWeb() {
  ...
  try {
    ...
    if (res.status === 503) {
      toast('GITHUB_TOKEN no configurado en Vercel — añádelo en Settings → Environment Variables.', 'error')
      return
    }
    if (!res.ok) throw new Error(data.error || `Error ${res.status}`)

    cacheListings()
    toast(`✓ Publicado — ${_listings.length} propiedades. La web se actualizará en ~30 s.`, 'success')
  } catch (e) {
